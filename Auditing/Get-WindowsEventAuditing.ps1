...
function Get-PowerShellAuditPolicy {
    ...
    if ($json) {
        ...
        $powershellauditpolicy.Add("ModuleNames", $(try {((Get-ItemProperty -Path 'HKLM:\SOFTWARE\Wow6432Node\Policies\Microsoft\Windows\PowerShell\ModuleLogging\ModuleNames') | ConvertTo-Csv)} catch {"Registry Key Not Found"}))
