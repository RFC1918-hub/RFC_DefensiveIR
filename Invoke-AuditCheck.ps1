...
function Get-PowerShellAuditPolicy {
    ...
    if ($json) {
        ...
        $powershellauditpolicy.Add("ModuleNames", $(try {(Get-ItemProperty -Path "HKLM:\SOFTWARE\Wow6432Node\Policies\Microsoft\Windows\PowerShell\ModuleLogging" -ErrorAction SilentlyContinue).ModuleNames} catch {"Registry Key Not Found"}))
    ...
    else 
    {
        ...
        try {
            write-host (Get-ItemProperty -Path "HKLM:\SOFTWARE\Wow6432Node\Policies\Microsoft\Windows\PowerShell\ModuleLogging" -ErrorAction SilentlyContinue).ModuleNames -ForegroundColor Green
        }
        catch {
            write-host "Registry Key Not Found" -ForegroundColor Red
        }
